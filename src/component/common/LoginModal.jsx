...
const LoginModal = ({ onClose, onLogin }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setApiError(err.response?.data?.message || (isLoginMode ? '잘못된 아이디 또는 비밀번호입니다.' : '회원가입에 실패했습니다.'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="modal-backdrop">
      <div className="modal-container" ref={modalRef}>
        ...
        <div className="modal-content">
          <h2 className="modal-title">{isLoginMode ? 'Teamo에 로그인 합니다' : 'Teamo에 오신 것을 환영합니다'}</h2>
          <p className="modal-subtitle">{isLoginMode ? '함께 할 팀원을 찾아보세요' : '새로운 팀과 함께 성장하세요'}</p>
          ...
          <div className="social-login-buttons">
            <button
              ...
              onClick={() => {
                ...
                // 리다이렉트 URI 인코딩
                const ENCODED_REDIRECT_URI = encodeURIComponent(KAKAO_REDIRECT_URI);
                const KAKAO_AUTH_URL = `${KAKAO_AUTH_URI}?client_id=${KAKAO_CLIENT_ID}&redirect_uri=${ENCODED_REDIRECT_URI}&response_type=code`;

                console.log('카카오 로그인 시도:', KAKAO_AUTH_URL);
                window.location.href = KAKAO_AUTH_URL;
              }}
            >
              <img src="/icons/kakao-icon.svg" alt="Kakao" className="social-icon" />
              카카오 로그인
            </button>
            <button
              ...
              onClick={() => {
                ...
                // 리다이렉트 URI 인코딩
                const ENCODED_REDIRECT_URI = encodeURIComponent(GOOGLE_REDIRECT_URI);
                ...
                console.log('구글 로그인 시도:', GOOGLE_AUTH_URL);
                window.location.href = GOOGLE_AUTH_URL;
              }}
            >
              <img src="/icons/google-icon.svg" alt="Google" className="social-icon" />
              구글 로그인
            </button>
            <button
              ...
              onClick={() => {
                ...
                // 리다이렉트 URI 인코딩
                const ENCODED_REDIRECT_URI = encodeURIComponent(GITHUB_REDIRECT_URI);
                ...
                console.log('Github 로그인 시도:', GITHUB_AUTH_URL);
            ...
            {isLoginMode ? (
              ...
              <>
                ...
                <p className="switch-mode">
                  계정이 없으신가요? <span onClick={handleSwitchToSignup}>회원가입하기</span>
                </p>
              </>
            ) : (
              // 회원가입 폼
              <>
                ...
                <div className="form-group">
                  ...
                  <input type="text" id="name" name="name" placeholder="이름을 입력하세요" value={formData.name} onChange={handleChange} required />
                  {inputError.name && (
                    <div
                      className="input-error"
                      style={{
                ...
                <p className="switch-mode">
                  이미 계정이 있으신가요? <span onClick={handleSwitchToLogin}>로그인하기</span>
                </p>
              </>
            )}
          </form>
