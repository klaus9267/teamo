  ...
  const handleSubmit = async e => {
    ...
    try {
      if (isLoginMode) {
        ...
        // 로그인 성공 시 토큰에서 사용자 정보 추출하여 userId 저장
        ...
        if (userInfo && userInfo.id) {
          ...
          console.log('로그인 성공: 사용자 ID 저장됨', userInfo.id);
        }
        // 응답 데이터에서 userId 추출 시도
        else if (res && res.userId) {
          localStorage.setItem('myUserId', String(res.userId));
          console.log('로그인 성공: 응답에서 사용자 ID 저장됨', res.userId);
        }
        // 모두 실패한 경우
        else {
          console.error('로그인 성공했지만 사용자 ID를 찾을 수 없음');
        }
