...
import { showSuccess, showError, showInfo, showWarning } from '../../utils/sweetAlert.ts';
...
export default function PostDetail() {
  ...
  const [applicationStatus, setApplicationStatus] = useState('none' as ApplicationStatus);
  ...
  const fetchApplicants = async postId => {
    ...
    try {
      ...
      console.log('받아온 지원자 데이터:', data); // 데이터 확인용 로그
      setApplicants(data);
  ...
  useEffect(() => {
    if (post && userId) {
      ...
      if (post.applies && Array.isArray(post.applies) && post.applies.includes(userId)) {
        setApplicationStatus('applied');
        console.log('이미 지원한 게시글입니다.');
      }

      // 2. matchedUsers 배열에 현재 userId가 포함되어 있는지 확인 (이미 합류한 상태)
      if (post.matchedUsers && Array.isArray(post.matchedUsers)) {
        const isUserMatched = post.matchedUsers.some(user => user.userId === userId);
        if (isUserMatched) {
          setApplicationStatus('accepted');
          console.log('이미 합류한 ' + (post.category || '프로젝트') + '입니다.');
        }
      }
    }
  }, [post, userId]);
  ...
  const handleApplicantAction = async (applyId, action) => {
    try {
      ...
      showError(`지원자 ${action === 'accept' ? '승인' : '거절'}에 실패했습니다.`);
    }
  };
  ...
  const handleApply = () => {
    if (!post) {
      showWarning('게시글 정보를 불러오는 중입니다. 잠시 후 다시 시도해주세요.');
      return;
    }
      ...
      <div className="post-detail-container">
        <div className="post-detail-main">
            ...
            <div className="applicants-modal-overlay">
              <div className="applicants-modal wider-modal">
                <div className="applicants-modal-header">
                  ...
                  <button className="modal-close-btn" onClick={toggleApplicantsList}>
                    ×
                  </button>
                </div>
                ...
                <div className="applicants-modal-content">
                  {applicants.length === 0 ? (
                    <p className="no-applicants-message">아직 지원자가 없습니다.</p>
                  ) : (
                    <div className="applicants-grid">
                      {applicants.map(applicant => (
                        <div key={applicant.applyId} className="applicant-card">
                          <div className="applicant-header">
                            <div className="applicant-score">{applicant.aiScore && <div className={`ai-score ${applicant.aiScore > 80 ? 'high-score' : applicant.aiScore > 60 ? 'medium-score' : 'low-score'}`}>AI 추천 {applicant.aiScore}점</div>}</div>
                            <div className="applicant-profile">
                              ...
                              <div className="applicant-info">
                                <div className="applicant-name">{applicant.nickname}</div>
                          ...
                          <div className="applicant-details">
                            <div className="applicant-section">
                              <h4>자기소개</h4>
                              <p>{applicant.resume && applicant.resume.content ? applicant.resume.content : '자기소개 내용이 없습니다.'}</p>
                            </div>
                            ...
                            {applicant.resume && applicant.resume.personality && (
                              <div className="applicant-section">
                                <h4>성향</h4>
                            ...
                            <div className="applicant-section">
                              <h4>포트폴리오</h4>
                              {applicant.resume && applicant.resume.portfolio && applicant.resume.portfolio !== 'NULL' && applicant.resume.portfolio !== 'null' ? (
                                <a href={applicant.resume.portfolio} target="_blank" rel="noopener noreferrer" className="portfolio-link">
                                  포트폴리오 바로가기
                                </a>
                              ) : (
                                <p className="no-portfolio-message">포트폴리오가 없습니다.</p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="applicant-actions">
                            {applicant.isSelected !== undefined ? (
                              ...
                              <>
                                <button className="accept-button" onClick={() => handleApplicantAction(applicant.applyId, 'accept')}>
                                  승인
                                </button>
                                <button className="reject-button" onClick={() => handleApplicantAction(applicant.applyId, 'reject')}>
                                  거절
                                </button>
                              </>
                            )}
          ...
          <section style={{ margin: '32px 0 0 0' }}>
            ...
            >
              <span style={{ fontWeight: 'bold', color: '#FFC107' }}>진행 방식: </span>
          ...
          <div className="team-members-section">
            ...
            <div style={{ marginBottom: 16, color: '#444' }}>
              ...
              >
                {post?.requirementPersonality || '특별히 필요한 성향이 없습니다.'}
              </p>
            </div>
          </div>
        ...
        {isAuthor ? (
          ...
          <div>
            {applicationStatus === 'applied' ? (
              ...
              <div className="accepted-status">합류한 {post?.category ?? '프로젝트'}입니다</div>
            ) : (
              <button onClick={handleApply} className="apply-button" disabled={applyLoading}>
                {applyLoading ? <Spinner size="small" color="#1B3A4B" inButton={true} /> : '지원하기'}
              </button>
            )}
          </div>
        )}
              ...
              <div className="apply-form">
                ...
                {resumesLoading ? (
                  <div style={{ textAlign: 'center', padding: '20px' }}>
                    <Spinner size="medium" text="자기소개서 목록을 불러오는 중..." />
                  </div>
                ) : resumesError ? (
                  <div
                  ...
                  <div className="resume-select-container">
                    {resumes.map(resume => (
                      <div key={resume.id} className={`resume-option ${selectedResume === resume.id ? 'selected' : ''}`} onClick={() => setSelectedResume(resume.id)}>
                        <div className="resume-option-title">{resume.title}</div>
                        <div className="resume-option-content">
                          {resume.content?.substring(0, 50)}
                          {resume.content?.length > 50 ? '...' : ''}
                        </div>
                        {selectedResume === resume.id && <div className="resume-selected-check">✓</div>}
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="no-resumes-message">
                    <p>등록된 자기소개서가 없습니다.</p>
                    <button className="create-resume-btn" onClick={() => navigate('/profile/resume/new')}>
                      자기소개서 작성하기
                    </button>
                  </div>
                )}
                ...
                <h3>지원 동기</h3>
                <textarea value={motivationText} onChange={e => setMotivationText(e.target.value)} placeholder="프로젝트에 지원하는 동기를 작성해주세요." className="motivation-textarea" rows={5} />

                <div className="apply-modal-actions">
                  <button className="cancel-btn" onClick={handleCloseModal}>
                    취소
                  </button>
                  <button className="apply-submit-btn" onClick={handleSubmitApplication} disabled={!selectedResume || !motivationText.trim() || submitLoading}>
                    {submitLoading ? <Spinner size="small" color="#1B3A4B" inButton={true} /> : '지원하기'}
                  </button>
                </div>
              </div>
            </div>
